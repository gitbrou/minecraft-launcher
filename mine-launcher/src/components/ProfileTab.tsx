...
export const ProfileTab: React.FC<ProfileTabProps> = ({ activeUsername }) => {
  const [stats, setStats] = useState<any>({
    ...
    worldsCount: 1,
    totalPlayTimeHours: '2.5',
    lastPlayedFormatted: 'Сегодня',
    favoriteWorld: 'Выживание 1.20',
    favoriteServer: 'Hypixel / PlayMine'
  })
  ...
  // Initialize 3D Interactive Skin Viewer (rotatable & zoomable with mouse)
  useEffect(() => {
    ...
    const viewer = new SkinViewer({
      ...
      width: 250,
      height: 480,
      skin: skinUrl || 'https://textures.minecraft.net/texture/31f477eb3753239a5f36e4f16b23d0c9fbf8f09d841e247b9015119a008c2a86'
    })
  ...
  return (
    <div style={{ flex: 1, display: 'flex', gap: '32px', alignItems: 'center', height: '100%', padding: '10px 20px', overflow: 'hidden' }}>
      {/* Left 3D Skin Rotatable Projection Card (Teal/Cyan Glowing Border as drawn in user image) */}
      <div
        style={{
          width: '270px',
          height: '490px',
        ...
        title="Зажмите мышь для 3D вращения персонажа"
      >
        <canvas ref={canvasRef} style={{ width: '100%', height: '100%', borderRadius: '20px', display: 'block' }} />
        <span style={{ position: 'absolute', bottom: '12px', fontSize: '11px', color: '#1e293b', fontWeight: 'bold', background: 'rgba(255,255,255,0.7)', padding: '2px 8px', borderRadius: '4px' }}>
          🔄 3D вращение мышью
        </span>
      </div>

      {/* Right Account Information Labels matching user mockup exact text */}
