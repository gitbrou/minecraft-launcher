  ...
  useEffect(() => {
    if (activeUsername && window.electronAPI) {
      ...
      window.electronAPI.getUserSkin(activeUsername).then((url) => {
        setSkinUrl(url || STEVE_TEXTURE_BASE64)
      })
    } else {
      setSkinUrl(STEVE_TEXTURE_BASE64)
    }
  }, [activeUsername])

  // Initialize 3D WebGL SkinViewer after small DOM render delay
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      try {
        viewer = new SkinViewer({
          ...
          width: 250,
          height: 460,
          skin: skinUrl || STEVE_TEXTURE_BASE64
        })
